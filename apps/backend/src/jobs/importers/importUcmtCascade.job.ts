...
export async function importarUCMT() {
  const startTime = Date.now();
  console.log('🚀 Iniciando importação do UCMT...');
  ...
  console.log(`✅ UCMT finalizado: ${totalProcessadas} registros processados em ${tempo} min`);

  const { importarUCBT } = await import('./importUcbtCascade.job.js');
  await importarUCBT();
}
